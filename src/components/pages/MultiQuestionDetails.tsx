...
const AnswerPicker = React.lazy(() => import("../common/AnswerPicker"));
      ...
      <PageTransition>
        <Suspense>
          <Link
            to="/tetelek/$id/questions"
            params={{ id: tetelId }}
            className="inline-flex items-center px-3 py-2 border border-border ml-10 rounded-md text-sm font-medium hover:bg-muted"
          >
      ...
      {isAuthenticated && isSuperUser ? (
        <>
          <button
            ...
            className="fixed md:bottom-2 bottom-14 right-2 p-3 bg-rose-600 text-white rounded-full 
          ...
          <Link
            ...
            className="fixed md:bottom-16 bottom-28 right-2 p-3 bg-blue-600 text-white rounded-full 
        ...
        <Link
          to="/tetelek/$id/questions/$qid/edit"
          params={{ id: tetelId, qid }}
          className="fixed md:bottom-2 bottom-14 right-2 p-3 bg-blue-600 text-white rounded-full 
               hover:bg-blue-700 transition-all hover:cursor-pointer transform hover:scale-105 flex items-center justify-center z-50"
          title="Szerkeszd a kérdést"
        >
          <FaPen size={20} />
